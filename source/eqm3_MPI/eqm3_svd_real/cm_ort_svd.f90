...
!  include 'types_eqm.inc'
  ...
  type(phonbase_typ), dimension (:), allocatable :: phonbs
  real(kind=4), dimension (:,:),allocatable :: d_mat,d_mat_sp,d_mat_sp_full
  real(kind=4), dimension (:), allocatable :: s,work
  real(kind=4), dimension (:,:), allocatable :: u,vt,a
  ...
  call sgesvd('A','A', dim_spur, dim_ind, d_mat_sp, dim_spur, s, u, dim_spur, vt, dim_ind, work, lwork, info)
  ...
  call sgemm('N','T',dim_spur_all,dim_ind,dim_ind,1.0,d_mat_sp_full,dim_spur_all,vt,dim_ind,0.0,a,dim_spur_all)
  
  write(*,*)'Test SVD'
  do i=1,dim_spur_all
  ! write(999,'(1000f10.5)')(a(i,j),j=1,dim_ind)
   do j=ns+1,dim_ind
     if (abs(a(i,j)).gt.0.000001) write(*,*)'Nonzero elem <spur| bas> !',i,j,a(i,j)
   enddo
  enddo
...
real(kind=4), dimension (:,:),allocatable :: d_mat,d_matc,d_inv
integer, dimension(:), allocatable :: nx,mxt
real(kind=4), dimension(:), allocatable :: work,e
...
!  a=d_matc 
 ...
 call SSYEV('V','L',dim_baser,d_matc,dim_baser,e,WORK,LWORK,INFO )
