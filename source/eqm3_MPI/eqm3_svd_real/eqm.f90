...
!     choleski arrays
      real(kind=4), dimension(:,:), allocatable :: dd,cq,d1,cdu,xr,vr,h_corr
      real(kind=8), dimension(:), allocatable :: wr
...
! calculation of matrices A and D
       ...
       CALL execute_command_line('./run_admat3_real.sh' )
...
!      CALL execute_command_line('rm ./scratch/d_mat*')
      ...
      call execute_command_line('./del_ad.sh')
!      call execute_command_line('""find ./scratch -name 'a_mat*' |xargs rm""')
!      call execute_command_line('find ./scratch -name 'd_mat*' |xargs rm')
