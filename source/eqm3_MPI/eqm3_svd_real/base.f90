!     last modification 7.1.2020  
      ...
      subroutine phonbase(nf,ipare,j,phonus,phonmus,idim1,idim2,idimbs,idphon,idphontr,phonbs,phon1,phon2,mxtr,n_spur)
...
!       phon2(i)%us=1
      ...
      write(892,*)
      write(892,*)'----------------------------------------------------------------'
      write(892,*)' Parity = ',ipare,'   J = ',j 
      write(892,*)'----------------------------------------------------------------'
      write(892,*)'  i  spur   1-ph: lamb    en_lam)    (n-1)-ph: gamma    en_gamma'
...
!          if (ilamup.eq.ilamcm.or.eila.gt.1000.0d0) phonbs(i)%spur=1  ! how to define spurius states?
    ...
    if (1.eq.1) then
      ...
      do i=1,idphon
        ...
        write(892,'(i5,i5,5x,i5,3x,f15.5,5x,i5,3x,f15.5)')i,phonbs(i)%spur,phonbs(i)%ilap,phon1(phonbs(i)%ilap)%enf,phonbs(i)%ila,phon2(phonbs(i)%ila)%enf
      enddo
      ...
      if (allocated(mxtr)) deallocate(mxtr)     
...
!         if ((phonbs(ii)%spur.eq.1.and.eila.le.xetrun).or.(phonbs(ii)%spur.eq.0.and.eila.le.xetrun)) then 
...
!         if ((eila+eilap).le.xetrun.and.ilamu.le.ilamup.and.ius1.eq.1.and.ius2.eq.1)  then 
               ...
               mxtr(jj)=ii
               nxtr(ii)=jj
         write(892,'(i5,i5,5x,i5,3x,f15.5,5x,i5,3x,f15.5)')ii,phonbs(ii)%spur,ilamup,eilap,ilamu,eila
         write(891,*)ilamu
        endif
