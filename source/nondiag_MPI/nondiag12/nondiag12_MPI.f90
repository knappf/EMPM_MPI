!     last update 14.6.2013
      ...
      call vintn12(2,ipar,jcal,levn,levp,myid,numprocs)

      call MPI_BARRIER(MPI_COMM_WORLD,ierr)

      if (myid ==0 ) then
            CALL execute_command_line('cat scratch/Vint12_* > Vint_phon12.dat' )
            CALL execute_command_line('rm scratch/Vint12_*' )      
      endif
